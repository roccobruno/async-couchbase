...
trait IndexApi extends BucketApi {
  ...
  def createIndex[T](fields: Seq[String]): Future[OpsResult] = {
    ...
    toFuture(asyncBucket.query(N1qlQuery.simple(stat))) map {
      resu =>
        OpsResult(isSuccess = resu.parseSuccess(),"") //TODO replace to use finalSuccess
    }
  }
  ...
  def findIndexes(): Future[List[CBIndex]] = {
    ...
    val query = N1qlQuery.simple(Select.select("indexes.*").from("system:indexes").where(i("keyspace_id").eq(s(asyncBucket.name()))))
